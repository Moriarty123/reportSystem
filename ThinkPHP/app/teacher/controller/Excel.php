...
class Excel extends Common
{
    ...
    public function reportExcel()
    {
		...
		//设置表头
       	$letter =explode(',',"A,B,C,D,E,F,G,H");
       	$arrHeader = array("实验课程", "实验任务", "实验报告", "提交学生", "提交结果", "批阅状态", "提交时间", "实验成绩");
       	$length =  count($arrHeader);
		...
		//表头赋值
       	...
       	foreach($reportList as $k=>$v){
       		...
       		$time = date("Y-m-d H:m:s", $v['submitTime']);
       		// dump($time);die();

       		$objActSheet->setCellValue('A'.$k, $v['courseName']);
       		$objActSheet->setCellValue('B'.$k, $v['taskName']);
       		$objActSheet->setCellValue('C'.$k, $v['reportName']);
       		$objActSheet->setCellValue('D'.$k, $v['studentName']);
       		$objActSheet->setCellValue('E'.$k, $v['submitResult']);
       		$objActSheet->setCellValue('F'.$k, $v['reviewStatus']);
       		$objActSheet->setCellValue('G'.$k, $time);
       		$objActSheet->setCellValue('H'.$k, $v['score']);
       		$objActSheet->getRowDimension($k)->setRowHeight(20);
       	}
       	$width = array(15, 20, 30, 40, 50);
       	//设置表格的宽度
